...
Windows 版本已提供 NSIS 打包目标、PowerShell 本机终端、新建 agent 会话的 WSL2 发行版／路径路由、标准 Windows Chrome/Edge 路径发现，以及默认禁网、资源限制、内置文件工具统一路由和私有任务副本补丁导入的显式 Docker 工具 profile。直接在 PowerShell 中运行的 agent 调用必须人工确认；普通 WSL2 路由仍是邻近宿主的执行方式，而显式开启的实验性 Bubblewrap Profile 会把同一批内置工具路由到原生私有任务副本，并通过审核式补丁导入返回。任务审核 checkpoint 与保守合并队列在重启后可恢复；完整边界见 [docs/windows-port.md](docs/windows-port.md)。
...
- **Mission Control 任务台账** — 展示持久化的任务生命周期事件、checkpoint、队列暂停原因、已检测改动路径，以及并行任务碰撞前的自愿路径声明重叠预警
...
- **Windows 运行器 Profile** — 新建会话可沿用启动环境、自动选择 WSL2、指定 WSL 发行版／挂载根，或使用 PowerShell；PiWin 会验证真实 WSL2 内核，并用 `wsl.exe --cd` 映射工作区。PowerShell 下的 agent `bash` 必须审批；普通 WSL2 仍是邻近宿主执行方式，显式开启的实验性 Bubblewrap Profile 则使用原生私有任务副本和审核式补丁导入
