...
- **受限 Docker 工具（Windows）** — 显式启用、默认禁网、容器根目录只读、移除 capabilities、进程/CPU/内存限制；`bash/read/write/edit/grep/find/ls` 共享只读或任务私有 volume，导入补丁并人工确认后才改宿主 worktree。宿主侧扩展/MCP 默认不加载；仅允许已审查的显式启用项逐次审批并记录审计日志
