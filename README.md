<p align="center">
  <img src="docs/logo.png" width="80" alt="PiWin Studio">
</p>

<h1 align="center">PiWin Studio</h1>

<p align="center">A Windows and WSL2-oriented desktop workbench for Pi coding agents.</p>
...
The Windows slice provides an NSIS build target, PowerShell as the local terminal, saved WSL2 distribution/path routing for new agent sessions, standard Windows Chrome/Edge discovery, and an explicit Docker tool profile with default-deny networking, resource limits, unified first-party file-tool routing, and private task-copy patch import. Direct PowerShell agent calls require human approval; ordinary WSL2 routing remains host-adjacent, while an explicit experimental Bubblewrap profile routes the same first-party tools through a native private task copy with reviewed patch import. Reviewed task checkpoints and a conservative merge queue survive app restarts; the complete boundary is documented in [docs/windows-port.md](docs/windows-port.md).
