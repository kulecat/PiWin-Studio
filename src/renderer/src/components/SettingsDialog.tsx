...
function ExecutionTab(): React.JSX.Element {
  const t = useT();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-start justify-between gap-3">
        <div>
          <div className="text-sm font-medium">{t("settings.executionTitle")}</div>
          <p className="pt-1 text-xs leading-relaxed text-fg-muted">{t("settings.executionIntro")}</p>
        </div>
        <button
          type="button"
          onClick={refresh}
          disabled={checking}
          className="inline-flex shrink-0 items-center gap-1.5 rounded-lg border border-border px-2.5 py-1.5 text-xs text-fg-secondary transition-colors hover:border-border-strong hover:bg-bg-hover disabled:opacity-50"
        >
          <RotateCcw size={12} className={checking ? "animate-spin" : undefined} />
          {t("settings.executionRefresh")}
        </button>
      </div>

      {!environment ? (
        <div className="flex items-center gap-2 rounded-xl border border-border bg-bg px-3 py-4 text-xs text-fg-muted">
          <Loader2 size={13} className="animate-spin" />
          {t("settings.executionChecking")}
        </div>
      ) : (
        <>
          <div className="rounded-xl border border-accent/30 bg-accent-muted/40 px-3 py-2.5 text-xs text-fg-secondary">
            <div>{t("settings.executionSelected", { runner: runnerLabel[environment.effectiveRunner] })}</div>
            <div className="pt-0.5 text-[11px] text-fg-muted">
              ...
              >
                <span
                  className={cn(
                    ...
                    runner.available ? "bg-success" : "bg-danger",
                  )}
                />
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 text-xs font-medium text-fg">
                    {runnerLabel[runner.kind]}
                    <span className={runner.available ? "text-success" : "text-danger"}>
                      {runner.available ? t("settings.executionReady") : t("settings.executionUnavailable")}
                    </span>
                  </div>
                  <div className="pt-1 text-[11px] leading-relaxed text-fg-muted">{runner.detail}</div>
                </div>
              </div>
            ))}
          </div>
          <p className="text-[11px] leading-relaxed text-fg-muted">{t("settings.executionHint")}</p>
        </>
      )}
    </div>
  );
...
const TAB_IDS = ["auth", "execution", "sandbox", "web", "deploy", "bark", "appearance", "about", "pi"] as const;
...
export function SettingsDialog(): React.JSX.Element | null {
  ...
  const tabs: { id: TabId; label: string; icon: React.ReactNode; ready?: boolean }[] = [
    ...
    { id: "execution", label: t("settings.tabs.execution"), icon: <MonitorPlay size={14} /> },
